...
@dashboards_router.post(
    "",
    response_model=HttpResponseModel[DashboardModel],
...
async def create_dashboard(
    ...
    with context.db_session() as db:
        ...
        if db_dashboard:
            raise HTTPException(
                status_code=400,
                detail=f"A dashboard with name {dashboard.name} already exists",
            )
    ...
    return http_response(
        status_code=status.HTTP_201_CREATED,
        data=DashboardModel.from_db_model(new_dashboard).model_dump(),
    )


@dashboards_router.get(
    "",
    response_model=HttpResponseModel[List[DashboardModel]],
